...
function docHTML(titreDoc, corps, pied) {
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ${corps}
  ${pied || `<div class="footer-bloc"><img src="${LOGO_IIP}" class="footer-logo" alt="IIP"/><div class="footer-texte">${ETAB.nom} · ${ETAB.adresse} · Fase ${ETAB.fase} · ${ETAB.site}</div></div>`}
</div></body></html>`;
}
...
export default function Disciplinaire() {
  ...
  useEffect(() => {
    chargerDossiers();
    af('/api/config/attestation_etab').then(d => { try { setEtabFooter(JSON.parse(d.valeur)); } catch {} }).catch(() => {});
  }, []);
  const piedHtml = (() => {
    const e = etabFooter || {};
    ...
    const texte = e.pied_page || [l1, l2].filter(Boolean).join('<br>') || (ETAB.nom + ' · ' + ETAB.adresse + ' · Fase ' + ETAB.fase);
    return `<div class="footer-bloc"><img src="${LOGO_IIP}" class="footer-logo" alt="IIP"/><div class="footer-texte">${texte}</div></div>`;
  })();
  ...
  const genConvocation = () => docHTML('Convocation à audition', `
    ...
    <div class="sig"><p>Le Directeur,<br><strong>${ETAB.directeur}</strong></p></div>`, piedHtml);
  ...
  const genPV = () => docHTML("PV d'audition", `
    ...
    <div class="sig"><table><tr>
      ...
      <td>L'étudiant·e,<br><br>__________________</td></tr></table></div>`, piedHtml);
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {STEPS.map((t, i) => (
          <button key={t} onClick={() => setStep(i + 1)}
            className={`text-xs px-3 py-1.5 rounded-champ border ${step === i + 1 ? 'bg-iip-blue text-white border-iip-blue' : 'bg-white text-gray-500 border-gray-300 hover:bg-gray-50'}`}>
            {i + 1}. {t}
          </button>
        ))}
      </div>
