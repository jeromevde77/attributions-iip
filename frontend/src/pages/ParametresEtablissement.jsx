...
export default function ParametresEtablissement() {
  ...
  /* La règle d'ajournement ne vit plus ici : elle a son composant, qui la lit
     et l'enregistre seul. La garder en double ferait qu'enregistrer l'adresse
     de l'école réécrirait une règle de délibération avec une valeur périmée —
     deux sources pour un même fait, et la pire des deux qui gagne. */
  ...
  return (
    <div className="max-w-[1100px] space-y-6">
      ...
      {/* LA RÈGLE D'AJOURNEMENT A REJOINT LE RÉFÉRENTIEL DE L'ANNÉE.
...
/**
 * CE QUE LE CONSEIL AJOURNE — une règle de l'établissement, pas du décret.
 *
 * Elle vivait au milieu de l'adresse et du matricule, dans « Identité et
 * sections » : on la cherchait dans Délibération, où elle n'était pas. Elle
 * décrit comment le Conseil délibère sur le référentiel, et elle s'enregistre
 * seule — sans quoi un réglage de délibération ne se sauvait qu'en passant par
 * le bouton d'un écran qui parle d'autre chose.
 */
export function ReglesDeliberation() {
  ...
  async function enregistrer() {
    ...
    try {
      const rep = await fetch('/api/config/deliberation_ajournement', {
        method: 'PUT', headers: authHeaders(),
        body: JSON.stringify({ valeur: JSON.stringify(ajour) }),
      });
      if (!rep.ok) throw new Error('refusé');
      setEtat('Enregistré ✓');
      setTimeout(() => setEtat(''), 3000);
    } catch (e) { setEtat('Erreur : ' + e.message); }
  }

  const Choix = ({ nom, valeur, options, onChoisir }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
      {options.map(o => (
        <label key={o.cle}
          className={`flex items-start gap-2.5 px-3 py-2 rounded-carte border cursor-pointer ${
            valeur === o.cle ? 'border-iip-blue bg-iip-blue/5' : 'border-slate-200'}`}>
  ...
  return (
    <section className="carte p-4 space-y-3 max-w-4xl">
      ...
      <Choix nom="portee" valeur={ajour.portee}
        onChoisir={v => setAjour(a => ({ ...a, portee: v }))}
        options={[
      ...
      {/* La forme de la seconde session ne se pose qu'en portée « acquis » : en
      ...
      {ajour.portee === 'aa' && (
        <div className="pl-4 border-l-2 border-iip-blue/30 space-y-2">
          ...
          <Choix nom="session2" valeur={ajour.session2}
            onChoisir={v => setAjour(a => ({ ...a, session2: v }))}
            options={[
